...
class Mlp(PyTorchModule):
    def __init__(
        ...
        self.layer_norm = layer_norm  # whether to normalize layers.
        self.fcs = []
        self.layer_norms = []
        in_size = input_size
...
class ModifiedMlp(PyTorchModule):
    def __init__(
        ...
        for i in range(self.tasks_num):
            ...
            self.last_layers[i] = lc
        lc = nn.Linear(last_layer_dim, output_size)
        lc.weight.data.uniform_(-init_w, init_w)
        lc.bias.data.uniform_(-init_w, init_w)
        self.__setattr__("lc-1", lc)
        self.last_layers[-1] = lc
    ...
    def forward(self, input, index, return_preactivations=False):
        ...
        preactivation = self.last_layers[index](output)
        output = self.output_activation(preactivation)
        if return_preactivations:
            return output, preactivation
        else:
